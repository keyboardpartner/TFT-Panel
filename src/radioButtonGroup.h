...
class RadioButtons : public GUIObject {
  ...
  RadioButtons(TFT_eSPI *tft, TouchProvider *touchProvider) : GUIObject(tft, touchProvider) { }
  ...
  void draw(uint16_t item) {
      ...
      my_uncheckcolor = TFT_BLACK; // Use dark grey for inactive RadioButtons
		...
		for (uint16_t j = 0; j < _count; j++) {
			...
			_tft->fillCircle(_x + radius, y_new, radius - _borderwidth, my_bgcolor);
