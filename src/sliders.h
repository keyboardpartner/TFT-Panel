...
class SliderHor : public GUIObject {

  public:
  //Switch(TFT_eSPI *tft, TouchProvider *touchProvider) {}
	SliderHor(TFT_eSPI *tft, TouchProvider *touchProvider) : GUIObject(tft, touchProvider) { }
...
class SliderVert : public GUIObject {

  public:
  //Switch(TFT_eSPI *tft, TouchProvider *touchProvider) {}
	SliderVert(TFT_eSPI *tft, TouchProvider *touchProvider) : GUIObject(tft, touchProvider) { }
  ...
  // Redraw of object, will be called when the object needs to be redrawn without state change
  void redraw(bool active) {
    _active = active; // Set active state
    draw(_level); // draw object
  }


	// Alive state overrides Active and Visible states.
	// Object will be ignored and not drawn if not enabled
  // Overload with redraw
  void setEnabled(bool enabled, bool redraw = false) {
    _enabled = enabled;
  ...
  // If the slider is pressed, it will enter a modal state,
  // updating the slider level, calling PressAction and redraw thumb control
  bool checkPressed(bool wait_released) {
    if (!_enabled) return false; // Do not check if not enabled
    if (_active && _touchProvider->pressed && contains(_touchProvider->tx, _touchProvider->ty)) {
      // Calculate the relative position of the touch within the slider
      bool first_touch = true;
      while (_touchProvider->checkTouch()) {
        _level = (float)(_thumb_end - _touchProvider->ty) / (float)(_thumb_travel);
        if (_level < 0) _level = 0; // Ensure level is not negative
        if (_level > 1.0) _level = 1.0; // Ensure level is not greater than maximum
        if (first_touch) {
          _level_integrator = _level;
  ...
  private:
	float _level, _level_integrator;
  int16_t _thumb_travel, _thumb_delta, _thumb_start, _thumb_end, _thumb_height, _thumb_width, _thumb_x;
  ...
  void resetCommon() {
    ...
    // slider/switch specific
	  _level    = 0.5;  // Default level
	  _level_integrator = 0.5;  // Default level
    _thumb_x      = _x + _borderwidth + 2;
    _thumb_height = _w - (_borderwidth * 2); // Calculate thumb width
    _thumb_width  = _thumb_height - 4; // Calculate thumb height
    _thumb_travel = _h - _thumb_height - (_borderwidth * 2) -4; // Calculate travel distance for the slider thumb
    _thumb_delta  = 0;  // Initialize thumb position
    _thumb_start  = _y + _thumb_height/2 + _borderwidth + 2; // Initialize thumb start position (center of thumb)
    _thumb_end    = _y + _thumb_travel + _thumb_height/2; // Initialize thumb end position
  }
